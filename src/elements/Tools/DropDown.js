...
const DropDown = (props) => {
  const { list01, list02, list03, list04 } = props;
  const [is_open, setIsOpen] = useState(false);

  return (
    <React.Fragment>
      <div className="relative inline-block">
        <Button onClick={() => setIsOpen(!is_open)}>
          <svg
        ...
        {is_open && (
          <div className="absolute right-0 z-20 w-48 py-2 mt-2 bg-white rounded-md shadow-xl dark:bg-gray-800">
            <Menu href="#">{list01}</Menu>

            <Menu href="#">view profile</Menu>

            <hr className="border-gray-200 dark:border-gray-700 " />

            <Menu href="#">{list02}</Menu>

            <hr className="border-gray-200 dark:border-gray-700 " />

            <Menu href="#">{list03}</Menu>
            <Menu href="#">로그아웃</Menu>
          </div>
        )}
      </div>
